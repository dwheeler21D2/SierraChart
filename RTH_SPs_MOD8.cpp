...
SCDLLName("RTH_SinglePrints_30m_Excess_And_Internal")
...
static bool ZoneFragmentHasMinimumDurationMinutes(
    ...
    std::vector<Zone>& outZones)
{
    ...
    else
    {
        ...
        const bool hasValidIndexSpan =
            (frozenHist.EndIndex >= frozenHist.StartIndex);

        const bool hasMinDuration =
            hasValidIndexSpan &&
            ZoneFragmentHasMinimumDurationMinutes(
                sc,
                frozenHist.StartIndex,
                frozenHist.EndIndex,
                minRedrawnFragmentMinutes
            );
...
SCSFExport scsf_RTH_SinglePrints_30m_Excess_And_Internal(SCStudyInterfaceRef sc)
{
    ...
    SCInputRef In_MinRedrawnFragmentMinutes = sc.Input[15];

    if (sc.SetDefaults)
    {
        sc.GraphName = "RTH 30m Single Prints (Atomic Band Logic)";
        sc.AutoLoop = 1;
        sc.GraphRegion = 0;
        ...
        In_MinRedrawnFragmentMinutes.Name = "Minimum Redrawn Fragment Width (minutes)";
        In_MinRedrawnFragmentMinutes.SetInt(60);
    ...
    const int minRedrawnFragmentMinutes = In_MinRedrawnFragmentMinutes.GetInt();
    ...
    // Finalize last RTH 30m period when leaving RTH
    if (i > 0 && !periods->empty())
    {
        Period30m& last = periods->back();

        if (!last.Closed && last.SessionType == 1 && sessionType != 1)
        {
            last.EndIndex = i - 1;
            last.Closed = true;
    ...
    const bool isRTH = (sessionType == 1);
